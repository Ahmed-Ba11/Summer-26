<script lang="ts">
     ...
     * من الإبهام، والبقيّة خلف «التفاصيل».
     */
    ...
    $effect(() => {
        ...
        const lastCategoryIsVisible = visibleCategories.some(
            (c) => c.id === lastCategoryId,
        );

        categoryId = lastCategoryIsVisible ? lastCategoryId : null;
    });
    ...
    const categoryMissing = $derived(
        mode === 'expense' && categoryId === null,
    );

    const onlyCategoryMissing = $derived(
        categoryMissing && amount > 0,
    );
    ...
    const isOther = $derived(
        selected?.name === 'أخرى' || selected?.icon === 'ellipsis',
    );

    const descriptionMissing = $derived(
        isOther && description.trim().length < 2,
    );

    const shortfall = $derived(
        mode === 'expense'
            ? Math.max(0, amount - availableToSpend(context))
            : 0,
    );
    ...
    const checks = $derived.by<RuleCheck[]>(() => {
        ...
        if (descriptionMissing) {
            list.unshift({
                ...
                detail:
                    'فئة «أخرى» تحتاج وصفاً، وإلا ما بتعرف وش كان هذا المصروف بعد أسبوع.',
            });
        }

        return shortfall > 0
            ? list.filter((c) => c.severity !== 'danger')
            : list;
    });

    const suggestion = $derived(
        checks.find((c) => c.suggestion) ?? null,
    );
    ...
    const fundingReady = $derived(
        shortfall === 0 ||
            (funding.source === 'savings' &&
                funding.savingsGoalId !== null) ||
            (funding.source === 'unlogged_income' &&
                funding.incomeAmount >= shortfall) ||
            funding.source === 'overspend',
    );

    const mustConfirm = $derived(
        needsConfirm(checks) && !confirmed,
    );

    const canProceed = $derived(
        amount > 0 && !blocked && !submitting,
    );

    const canSave = $derived(canProceed && fundingReady);

    $effect(() => {
        if (shortfall === 0 && step === 2) {
            step = 1;
        }
    });
    ...
    const preview = $derived.by(() => {
        ...
        return {
            ...
            after,
            dailyBefore:
                context.daysUntilSalary > 0
                    ? Math.floor(
                          before / context.daysUntilSalary,
                      )
                    : 0,
            dailyAfter:
                context.daysUntilSalary > 0
                    ? Math.floor(
                          after / context.daysUntilSalary,
                      )
                    : 0,
        };
    });
    ...
    function reset() {
        ...
        categoryId = lastCategoryIsVisible
            ? lastCategoryId
            : null;
            ...
            url,
            {
                ...
                income_date:
                    mode === 'income'
                        ...
                        : undefined,
                funding_source:
                    mode === 'expense' && shortfall > 0
                        ? funding.source
                        : undefined,
                savings_goal_id:
                    mode === 'expense' && shortfall > 0
                        ? funding.savingsGoalId ??
                          undefined
                        : undefined,
                income_amount:
                    mode === 'expense' && shortfall > 0
                        ? funding.incomeAmount ||
                          undefined
                        : undefined,
                income_source:
                    mode === 'expense' && shortfall > 0
                        ? funding.incomeSource ||
                          undefined
                        : undefined,
            } as never,
            {
...
>
    ...
    {:else}
        <div class="flex flex-col gap-3">
            <!-- مصروف / دخل -->
            <div
                class="flex rounded-2xl border border-border bg-secondary p-[3px]"
            >
                <button
                    type="button"
                    aria-pressed={mode === 'expense'}
                    class="min-h-10 flex-1 rounded-xl text-[12.5px] transition-colors {mode ===
                    'expense'
            ...
            <div class="text-center">
                <p
                    class="text-[42px] leading-none font-semibold tracking-[-0.04em] tabular-nums"
                >
                    {#if raw}
                        {formatAmount(amount)}
                    {:else}
                ...
                >
                    {#each visibleCategories as c (c.id)}
                        <button
                            ...
                            class="flex min-w-0 flex-col items-center justify-center gap-0.5 rounded-xl border px-0.5 py-1.5 transition-colors {categoryId ===
                            c.id
                                ? 'border-current'
                                : 'border-border'}"
            ...
            {#if preview}
                <div
                    class="flex items-center justify-between gap-2.5 rounded-2xl border border-border bg-secondary px-3 py-2 text-[11.5px]"
                >
                    <span
                    ...
                    <span
                        class="whitespace-nowrap text-foreground/75"
                    >
                        <b
                        ...
                        <span
                            class="mx-0.5 text-input"
                        >
                            ←
                        </span>

                        <b
                            class="font-semibold tabular-nums {preview.after <
                            0
                                ? 'text-destructive'
                                : 'text-foreground'}"
                        >
            ...
            {#if warnings.length}
                ...
                <div
                    class="rounded-2xl border px-3 py-2 text-[11.5px] {st.box}"
                >
                    <div
                    ...
                    >
                        {#if top.severity === 'info'}
                            <Info
                                class="mt-0.5 size-[17px] shrink-0 {st.icon}"
                                stroke-width="1.9"
                            />
                        {:else}
                            <TriangleAlert
