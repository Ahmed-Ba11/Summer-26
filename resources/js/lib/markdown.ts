...
/** يحوّل نصّاً إلى ما يُعرَض حرفياً، مع إبقاء الكيانات المكتوبة أصلاً. */
...
const marked = new Marked({
    ...
    renderer: {
        /**
         * أي HTML في نصّ الموديل يُعرض **حرفاً** لا يُنفَّذ.
         *
         * ══════════════════════════════════════════════════════════════
         *  الطبقة الأولى من طبقتين. `DOMPurify` هي الثانية.
         * ══════════════════════════════════════════════════════════════
         *
         * المصدر الوحيد لـHTML في رد الوكيل هو بيانات المستخدم نفسها
         * منعكسةً — وصف عملية كُتب فيه `<img src=x onerror=…>`. الوكيل
         * لا ينتج HTML من عنده، فلا شيء يُفقَد بمنعه.
         *
         * التعقيم وحده لا يكفي هنا: `DOMPurify` يجرّد `onerror` ويُبقي
         * `<img>`، فيظهر الوصف صورةً مكسورة ويُطلق طلب شبكة لعنوانٍ من
         * صنع المستخدم. آمن، لكنه ليس «يظهر كنصّ عادي». (رصده فحص
         * المتصفّح فعلياً — `tests/browser/assistant.mjs`.)
         */
        html({ text }) {
